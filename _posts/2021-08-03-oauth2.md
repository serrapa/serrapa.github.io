...
> Requirements.txt:
  - OAuth2==what_is&how_works
  - Open Redirect==what_is is
  - XSS==what_is
  - CSRF==what_is is
...
Obviously, the most exciting part was the last one. After buckle up, I visited a website well-known to be a Bug Bounty platform and looked for a great program to get some bounties. I got an Italian website of an HR consulting firm that was providing a registration process via OAuth2 with Linkedin. But what was strange and uncommon? The website was using a third-party Javascript library to implement the OAuth2 process developed by another company. After having spent some time understanding how that library works, I just understood that it was a sort of interface which actually managed the OAuth process. What does it mean? Well, a bounty there means a bounty wherever this library is used.

Since that company, which developed the library, had neither a bounty program nor a responsible disclosure program (but a very good and polite Information Security Officer), I decided to get in touch with them anyway to report the vulnerabilities I found. What's the moral of this story?  I passively made them open a responsible disclosure program! This is the best achievement I ever got, so proud of it! :D
...
![Window shadow](/images/oauth_flow.png){: .shadow width="1548" height="864" style="max-width: 90%" }
_OAuth 2 Flow_
...
```
***2° Vuln Got !!!***: the payload was reflected in the response, allowing an attacker to perform an XSS or other types of injections attacks with all consequences of these types of attack.

![Window shadow](/images/xss_browser.png){: .shadow width="1548" height="864" style="max-width: 90%" }
_XSS_

In a normal scenario, the response page contained a form, which was broken because of the part of the payload after the double-quotes. The purpose of this page was to send the user’s data obtained from LinkedIn to the main application (domain_A).
This page might be considered as an interface between the OAuth2 process and the application, but because of this vulnerability, it could be controlled by an attacker, who might trick the victim by creating a page under the domain of home.\<domain_B\>.nl (HTML Injection), sends fake or additional user’s data beyond the name, surname and email, initiates interactions with other application users, including malicious attacks, that will appear to originate from the initial victim user or exploits XXE Injection attacks to retrieve files or exfiltrate data from the main application.
