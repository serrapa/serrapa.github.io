...
Obviously, the most exciting part was the last one. I came through an Italian website owned by a HR consulting firm that provides a registration process via OAuth2 (with Linkedin) in a uncommon way: a third party Javascript library developed by another company was used. Why uncommon? Well,
...
The request was created by the vulnerable javascript (we will see later why vulnerable). It takes several parameters before going through the OAuth flow, here those which got my attention:
- **method**: it stood for the Resource Server (even though Linkedin was the only one available for the italian website, by looking into the javascript there were others providers available: Dropbox, GooglePlus, Facebook and so on)
- **token**: it could be the ID related to the main web site
...
The exploitation started in this request: if an error occured, the user was redirected to the redirectOnError value, so my first attempt was just changing that value with a site of mine. To do it, I used a third online service created also for these kinds of purposes: Beeceptor. As the last step, I had to change something to arise an error, so I deleted the token parameter from the request. At the end, the crafted request was like:
...
***1° Vuln Got !!!***: the response was a 302 Redirect to the endpoint inserted in
...
I suggested to ensure that the supplied value (***redirectOnError***) is valid, appropriate for the application and authorized for the user. It is important to notice that the input should be sanitized, by checking that the ***redirectOnError*** parameter redirects to the same site which made the wrong request, even if the input parameter has been changed by an attacker.
...
The attack scenario starts from the same request as before, but now the attention is given to the ***postProfileUrl*** parameter and the scenario becomes a little bit more complex than the previous one because of the usage of OAuth. Modifying the ***postProfileUrl*** and sending the request, the user is gonna follow two redirects.
The following is the crafted request:
...
The part after the double quotes, them included, concerns the Reflected Cross Site Scripting founded after the LinkedIn Authentication process. The response that follows is the first redirect:
...
connection: close
```
As we can notice, the response is a 302 redirect to the value of the location header. Once the redirection is done, the user will be redirected to the LinkedIn page (second redirect), where OAuth2 starts:
...
After doing the authentication and granting the access to his LinkedIn data, the user will be redirected to the ***redirect_uri*** parameter.

Here something starts to happen: if the authentication succeeds, LinkedIn issues the code parameter for the endpoint home.<domain_B>.nl, so that it will be able to exchange that parameter with a token necessary to request the LinkedIn user’s data specified by the scope parameter. Diagrams are better than words:

![Window shadow](/images/flowOauth.png){: .shadow width="1548" height="864" style="max-width: 90%" }
_OAuth 2 Flow_

The figure above shows the OAuth2 Authorization Code Flow after the authentication/authorization. Once the user is done, the browser redirects him to https://home.<domain_B>.nl/<path>/importLinkedInProfile?code=XXX&state= YYY, where a html form sends user’s data to the main application (domain_A) in XML format:
```
...
```
***2° Vuln Got !!!***: the payload is reflected in this response page, allowing an attacker to perform an XSS or other types of injections attacks with all consequences of these types of attack.

![Window shadow](/images/xss_browser.png){: .shadow width="1548" height="864" style="max-width: 90%" }
_XSS_

In the normal flow of the third party javascript, the response page contains a form, which has been broken because of the part of the payload after the double quotes. The purpose of this page is sending the user’s data obtained from LinkedIn to the main application (domain_A).
This page might be considered as an interface between the OAuth2 process and the application and because of this vulnerabilities, it could be controlled by an attacker, who might trick the victim by creating a page under the domain of home.<domain_B.nl (HTML Injection), sends fake or additional user’s data beyond the name, surname and email, initiates interactions with other application users, including malicious attacks, that will appear to originate from the initial victim user or exploits XXE Injection attacks to retrieve files or exfiltrate data from the main application.


### Impact


The previous section has shown how an open redirect might result in a critical scenario (the previous one described is an example!). Considering that the javascript offered by Domain_B is used as a third-component by several sites, so no matter if it is just an open redirect because any scenario is possible.
...
***One shot, Two kills***: This scenario is highly critical because it might ruin the image of Domain_B and the image of the web site which is using the vulnerable module.
...
## Bypass CSRF-Token on OAuth2

To reproduce an CSRF attack, I needed to build a malicious server that the
victim should visit through a link sent by me (attacker). Although the victim
might avoid clicking the link because it is from a untrusted source, an attacker
...
![Window shadow](/images/attacker_server.png){: .shadow width="1548" height="864" style="max-width: 90%" }
_Malicious Server_

The HTML source code of this server is pretty simple, just because it is a demonstration but in a real scenario the attacker should create a trusted page for the victim:
...
<html>
  <body>
    ...
    <script>
      ...
      function myFunction () {
        window.open ("https://home.<domain_B>.nl/<path>/importProfile?method=linkedin&token=<id_client>&doValidation=true&redirectonError=https%3A%2F%2Fextranet.<domain_A>.it%2F<path>&postProfileUrl=https%3A%2F%2Fextranet.<domain_A>.it%2FC<path>&postoriginalDocument=true","_blank","toolbar=yes,scrollbars=yes, resizable=yes, top=5,left=50,width=40, height=40");
        sleep (4000);
        // This fuction is needed to get a valid session in the Textkernel context.
      }
      function myFunction2 () {
        document.location="https://home.textkernel.nl/carerix/public/importLinkedInProfile?code=XXX&state=TGlua2VkSW5BbnRpQ1NSRlNlY3JldA"
        sleep (4000);
      }
```

In order to make the attack succeed, the myFunction1() function is needed to
get a valid session, otherwise, the request to /<path>/importLinkedProfile?code=XXX&state=XXX fails.
Once the malicious page is loaded, the first function should be run: for this PoC I have implemented two buttons to perform the two function, but in a real scenario the action of clicking can be made via Javascript without any user interaction.
...
- ***code***: this parameter represents the OAuth code related to the account signed in. To link the attacker’s account to the victim’s session it is necessary that this code belongs to the attacker. To get it I have followed these steps: as the attacker, I used my LinkedIn test account as I was doing the registration in the page of domain_A, but I dropped the request on Burp Suite once I got the code parameter (the figure below shows the request with the code):
...
Dropping that request means that the code was still valid, so the last step was just copying it in the right place on myFunction2:
...
- ***state***: this parameter represents an Anti-CSRF token in OAuth and should be unguessable and validated every time you use it.

Once the myFuction2() is executed, the victim will be redirected to the registration page in Domain_A with the attacker’s data:


![Window shadow](/images/csrf_final2.png){: .shadow width="1548" height="864" style="max-width: 90%" }
_CSRF_

The blue box under the LinkedIn button means that the connection with LinkedIn succeeded without any problems (it is just written in Italian). Another confirmation was given by looking into the source code.

***Final Consideration***: The attack was possible because the value of the ***state*** parameter always remained the same: TGlu______________3JldA. The state parameter for CSRF mitigation in OAuth2 on the redirection endpoint means that within its value there is a unique and non-guessable value associated with each authentication request about to be initiated. That unique and non-guessable value allows to prevent the attack by confirming if the value coming from the response matches the one expected (that generated when initiating the request).


### Impact

The impact of this attack could be really serious because you are linking the attacker’s account to the victim’s session and this means that an attacker can see what the victim is doing: if some sensitive data are added to the profile, the attacker will be able to see them because their will be in his account.
Given the fact that the vulnerable javascript used was a third-party component, it should have prevented CSRF-attacks, because every critical scenario is possible given the high number of web apps which might use it. Try to imagine if the victim is typing a credit card number or uploading a CV without realising that he/she is logged in the attacker’s account.
...
The solution was simple: before sending user to the provider, generate a random nonce and link it to the user session. When the user was back, the domain_B needed to make sure the state received was equal to the previous one generated.
